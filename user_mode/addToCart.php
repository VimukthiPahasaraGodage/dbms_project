...
if($_SERVER['REQUEST_METHOD'] == "POST")
	{
      ...
      if($roww['quantity']>0){
        $query = 'insert into cart_product (customer_id,product_id,quantity) values ('.$customer_id.','.$product_id.',1)';
...
<?php 
        ...
        $max = $roww['quantity'];
        $ma = 'select * from cart_product where (product_id='.$_GET['id'].' and customer_id='.$_SESSION['customer_id'].')';
        $ma= $con->query($ma);
        if($ma->num_rows>0){
          $ma = $ma->fetch_assoc();
          $max = $max - $ma['quantity'];
				...
				<div  class="card container border" style="max-width:700px;height:auto;background: linear-gradient(rgba(21, 51, 2, 0.8),rgba(185,220,188,0.8));border: white groove 10px; border-radius: 12px;padding-bottom:10px;color: #3fee14;">
				<h2 style="text-align: center; color: #3ce811; font-weight: bolder;" style="color:green;text-align:center;">'.$roww['product_name'].'</h2>
        ...
        <form action="'.$_SERVER['REQUEST_URI'].'"  method="post" class="was-validated">
          ...
          <button value="Signup" name="signup" style="float:right;" type="submit" class="btn btn-primary">Add To Cart</button>

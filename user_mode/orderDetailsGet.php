...
<script>
    ...
    function getRoute(id) {
        var tr = document.getElementById("route");
...
<form method="post" action="orderDetailsGetFinal.php" class=" was-validated" style="padding: 0;" >
<div class="mb-3">
    <label for="address" class="form-label">Expected Delivery Date:</label>
    <input style="width:40%;" href="cart.php" name="expectDate" onfocus="this.min=leastDate()" type="date"  class="form-control" id="address" placeholder="Enter Address" name="address" required>
</div>

<div class="mb-3">
    <label for="address" class="form-label">Nearby Store:</label>
    <select required onchange="this.form.submit();"  class="form-control" id="sel2" id="sel2" name="nearStore">
        ...
        while ($row=$store->fetch_assoc()){
            echo ('
                        <option   value="'.$row['store_id'].'">'.$row['station'].'</option>
                        ');
        }
        ?>
    </select>
...
<button style="width:40%;float:right;"  name="signup" type="submit" class="btn btn-success">Confirm Order</button>
</form>

...
if($_SERVER['REQUEST_METHOD'] == "POST")
	{
      ...
      if($roww['quantity']>0){
        $query = 'insert into card_product (customer_id,product_id,quantity) values ('.$customer_id.','.$product_id.',1)';
...
<?php 
        ...
        $max = $roww['quantity'];
        $ma = 'select * from card_product where (product_id='.$_GET['id'].' and customer_id='.$_SESSION['customer_id'].')';
        $ma= $con->query($ma);
        if($ma->num_rows>0){
          $ma = $ma->fetch_assoc();
          $max = $max - $ma['quantity'];
        ...
        <form action="'.$_SERVER['REQUEST_URI'].'"  method="post" class="was-validated">
          ...
          <button value="Signup" name="signup" style="float:right;" type="submit" class="btn btn-primary">Add To Card</button>
